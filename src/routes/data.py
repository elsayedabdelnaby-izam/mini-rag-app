...
async def upload_data(project_id: str, file: UploadFile = File(...), app_settings: Settings = Depends(get_settings)):
    ...
    file_path, file_id = data_controller.generate_unique_filepath(org_filename=file.filename, project_id=project_id)
    ...
    try:
        ...
        return JSONResponse(status_code=status.HTTP_400_BAD_REQUEST, content={"message": ResponseEnum.FILE_UPLOAD_FAILED.value})
    return JSONResponse(
...
async def process_endpoint(project_id: str, process_request: ProcessRequest):
    ...
    if file_content is None:
        return JSONResponse(
            ...
            content={"message": ResponseEnum.PROCESSING_FAILED.value},
        )
